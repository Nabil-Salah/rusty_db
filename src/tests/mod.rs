...
fn test_buffer_pool_with_disk_manager() {
    ...
    let (page_id1, page1) = buffer_pool.new_page().unwrap();
    let (page_id2, page2) = buffer_pool.new_page().unwrap();
...
fn test_concurrent_page_access() {
    ...
    let page_id = {
        let (id, _) = buffer_pool.lock().unwrap().new_page().unwrap();
        id
    };
...
fn test_buffer_pool_recovery() {
    ...
    // Second session: Recover and verify data
    {
        ...
        // Read page 0
        let page1 = buffer_pool.fetch_page(0).unwrap();
        {
        ...
        // Read page 1
        let page2 = buffer_pool.fetch_page(1).unwrap();
        {
